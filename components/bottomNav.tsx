      ...
      {navItems.map((nav) => {
        ...
        return (
          <Link
            key={nav.id}
            href={nav.link}
            className="flex-1 flex flex-col items-center justify-center text-xs text-gray-500 hover:text-purple-600 transition"
          >
            <div
            ...
            <span
              className={`${isActive ? "text-purple-600 font-medium" : ""}`}
            >
              {nav.label}
            </span>
          </Link>
